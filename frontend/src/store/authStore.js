...
const getSavedAuth = () => {
  try {
    const saved = localStorage.getItem('campuspay_auth');
    if (saved) {
...
export const useAuthStore = create((set) => {
  ...
  return {
    ...
    login: (user, token, refreshToken) => {
      localStorage.setItem('campuspay_auth', JSON.stringify({ user, token, refreshToken }));
      set({ user, token, refreshToken, isAuthenticated: true });
    },
    
    logout: () => {
      localStorage.removeItem('campuspay_auth');
